...
function scssTask() {
  return (
    src(files.scssPath)
      ...
      .pipe(dest("dist"))
  ); //destination folder for final compiled files
}
...
function optimizeImg() {
  return src(files.imagePath).pipe(dest("dist/images"));
}
...
function watchTask() {
  ...
  watch(
    ...
    parallel(scssTask, jsTask, optimizeImg, browsersyncReload)
  ); //takes 2 params. 1st param cite the file you want to monitor. 2nd param run that file that you cite to monitor using Gulp syntax (either series or parallel)
}
